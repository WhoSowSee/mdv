use super::{
    ...
    Result, ThemeElement, WrapMode, as_24_bit_terminal_escaped, as_palette_terminal_escaped,
    create_style, detect_source_code,
};
...
impl<'a> EventRenderer<'a> {
    ...
    pub(super) fn highlight_code(&self, code: &str, language_hint: Option<&str>) -> Result<String> {
        ...
        for line in LinesWithEndings::from(code) {
            ...
            let escaped = if self.code_theme.uses_palette() {
                as_palette_terminal_escaped(&ranges[..], &self.code_theme.palette_map, false)
            } else {
