...
impl<'a> EventRenderer<'a> {
    pub(in crate::renderer::event) fn handle_table_end(&mut self) -> Result<()> {
    ...
    pub(in crate::renderer::event) fn render_table(
        &mut self,
    ...
    pub(in crate::renderer::event) fn render_embedded_table(
        &self,
        mut table: TableState,
    ) -> Result<String> {
        if !self.prepare_table(&mut table) {
    ...
    pub(super) fn prepare_table(&self, table: &mut TableState) -> bool {
    ...
    pub(super) fn expand_html_table_horizontal_rules(&self, table: &mut TableState) {
        if !table
            .headers
            .iter()
