...
- `-L, --interactive` — открыть интерактивный браузер документов. Запуск `mdv` без аргументов открывает текущий каталог, а путь к каталогу открывает указанный каталог. Браузер рекурсивно находит Markdown-файлы с учётом скрытых путей и `.gitignore`.
...
- `-D, --pretty-list <стиль>` — заменяет стандартные маркеры `-` неупорядоченных списков на набор иконок по уровням. Значение обязательно и задаётся в формате `type:<nerd-font|unicode>;size:<large|small>`, например `--pretty-list 'type:unicode;size:small'`.
...
- `-N, --uniform-list-marker <level:1-4|icon:глиф>` — использует один маркер на всех уровнях вложенности (только с `--pretty-list`). `level:2` берёт второй маркер выбранного набора, а `icon:*` задаёт собственный глиф.
...
- `-a, --margin <right:N;left:N>` — задаёт левый и/или правый отступ. Неуказанная сторона равна `0`.
...
- `pager_status_bar_transparent` (необязательно) — `false` сохраняет фон статус-бара пейджера; `true` убирает фон и разделяет секции символом `|`. Если поле не указано, значение наследуется от базовой темы.
