...
- `-s, --code-block-style <simple|pretty>[:show-icons;icon-only]` — стиль оформления блоков кода: одинарная граница или рамка. `show-icons` добавляет иконку языка к подписи, `icon-only` показывает только иконку, скрывая текст языка (по умолчанию `pretty`).
...
- `-O, --callout-style <pretty|simple>[:show-icons;fold-icons;label-inside;uppercase]` — стиль callout-блоков и правила отображения подписи. Для корректного отображения иконок нужны шрифты Nerd Fonts в терминале.
  `label-inside` поддерживается только с `pretty`, `fold-icons` требует `show-icons`.
- `-U, --custom-callout <name:icon=...,color=...;...>` — переопределение или добавление подписей callout.
  `icon` и `color` можно указывать по отдельности; формат цвета как в `--custom-theme`.

### Чекбоксы

- `-P, --pretty-checkbox <square|circle>` — рендерит чекбоксы task-list как иконки Nerd Font вместо стандартных `[ ]` / `[x]`. Требует шрифт Nerd Font в терминале.
- `-B, --custom-checkbox <char>:<icon>[:<color>];...` — переопределяет встроенные иконки чекбоксов или добавляет новые состояния (только с `--pretty-checkbox`). `<char>` — символ чекбокса (`, `, `x`, `*`, `?`, `!` и т.д.); `<icon>` — новая иконка; `<color>` — опциональный цвет. Цвета принимают именованные, hex, rgb и `ansi(N)`.
  - Переопределение:  `-B ' :icon'`         заменить иконку неотмеченного
  - Добавление:       `-B '*:icon'`         добавить состояние `[*]`
  - Цвет:             `-B ' :icon:yellow'`  или `#ff0000`, `128,1,1`, `ansi(200)`
  - Без иконки:       `-B '?:red'`          оставить иконку `[?]`, применить красный

### Списки

- `-D, --pretty-list` — заменяет стандартные маркеры `-` неупорядоченных списков на иконки Nerd Font для каждого уровня вложенности. Требует шрифт Nerd Font в терминале.
- `-Q, --custom-list <уровень>:<иконка>[:<цвет>];...` — переопределяет иконку и/или цвет маркера для конкретных уровней вложенности (только с `--pretty-list`). Уровень — 1-based; иконка — глиф маркера. Цвета принимают именованные (`red`), hex (`#ff0000`), rgb (`255,0,0`) и `ansi(N)`.
...
- `-o, --footnote-style <endnotes|attached>` — размещение сносок в конце документа или после каждого абзаца.
- `-M, --missing-footnote-style <show|hide>` — поведение для отсутствующих, некорректных или пустых определений сносок.
...
Тема `terminal` по умолчанию использует собственный цвет переднего плана терминала (`\x1b[39m`) для обычного текста и кода, поэтому адаптируется к светлому и тёмному фону. Цветовые акценты синтаксиса используют палитру терминала вместо фиксированного truecolor RGB. Темы с `rgb(...)` (например `monokai`) и внешние Syntect `.tmTheme` через `--code-theme` по-прежнему используют truecolor там, где это нужно.
...
- Любое цветовое поле необязательно и наследуется от базовой темы. Доступны UI-поля: `text`, `text_light`, `h1`..`h6`, `code`, `code_block`, `quote`, `link`, `emphasis`, `strong`, `strikethrough`, `highlight_background`, `background`, `border`, `list_marker`, `table_header`, `table_border`, `error`, `warning`.
...
Пользовательская тема с тем же именем, что и встроенная, имеет приоритет и полностью её заменяет — это поддерживаемый способ сделать форк встроенной темы, не копируя все 33 поля. Нечитаемые или повреждённые файлы пропускаются с предупреждением, поэтому одна битая тема не ломает остальные.
