...
- `-s, --style-code-block <simple|pretty>` — стиль оформления блоков кода: одинарная граница или рамка (по умолчанию `simple`).
...
- `-L, --show-code-language` — показ подписи языка над блоками кода, если она доступна.
